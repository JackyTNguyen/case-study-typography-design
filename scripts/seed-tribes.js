// Fetches the Exactitudes tribe index once and stores it in data/tribes.json.
// The server only ever reads this file. Re-run by hand (npm run seed) to pick
// up newly published series.
const fs = require('fs');
...
(async () => {
  ...
  if (!tribes.length) throw new Error('Index was empty; leaving data/tribes.json untouched');

  fs.mkdirSync(path.dirname(OUT), { recursive: true });
  fs.writeFileSync(OUT, JSON.stringify({ fetchedAt: new Date().toISOString(), tribes }, null, 2));
  console.log(`Wrote ${tribes.length} tribes to data/tribes.json`);
})().catch((err) => {
