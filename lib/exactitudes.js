// Live, per-match fetches from exactitudes.com. Images are only ever held in
// memory for compositing; nothing here touches the disk.
...
async function get(url, as) {
  ...
  if (res.status === 429) throw new AppError('RATE_LIMITED', 'exactitudes.com is rate limiting', 429);
...
// Every portrait in the series, uncapped. The API returns paths like
// "https://exactitudes.com/wp/../media/..." so each is normalised via URL().
async function fetchSeriesImages(wpId) {
  ...
  if (!urls.length) throw new AppError('SOURCE_CHANGED', 'Series contains no usable images');
  return urls;
}

// Image bytes for the composite, a few at a time. A broken image becomes null
