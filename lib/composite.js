...
const { barHeight, grid } = require('../public/js/layout');
...
async function composite(match, visitorJpeg, viewport) {
  ...
  const { cols, rows } = grid(cells.length, vw, vh - barHeight(vw, vh));
  const gridH = H - B;
  ...
  const tiles = await Promise.all(
    Array.from({ length: cols * rows }, async (_, i) => {
      ...
      const top = B + Math.round((r * gridH) / rows);
      const w = Math.round(((c + 1) * W) / cols) - left;
      const h = B + Math.round(((r + 1) * gridH) / rows) - top;
      const buf = cells[i];
      let input;
      try {
        input = buf
