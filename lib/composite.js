...
// visitor: Buffer (the archive-framed cut-out, or the original photo as fallback)
async function composite(match, visitor, viewport) {
  ...
  const photos = await Promise.all(
    rects.map(async (r, i) => {
      ...
      let input;
      try {
        if (cells[i]) input = await sharp(cells[i]).rotate().resize(w, h, { fit: 'cover', position: 'centre' }).toBuffer();
