...
const { isPortrait, barHeight, tiles, fitVisitor } = require('../public/js/layout');
...
// The visitor's square photo placed with fitVisitor(): fills the cell like the
// archive photos unless that would crop the person, then shrinks to show all
// of them on matching grey.
async function visitorTile({ buf, box }, w, h) {
  const f = fitVisitor(w, h, box);
  const size = Math.round(f.size);
  const x = Math.round(f.x);
  const y = Math.round(f.y);
  const photo = await sharp(buf).rotate().resize(size, size, { fit: 'cover' }).toBuffer();
  // Keep only the part of the photo that lands inside the cell.
  const left = Math.max(0, -x);
  const top = Math.max(0, -y);
  const width = Math.min(size - left, w - Math.max(0, x));
  const height = Math.min(size - top, h - Math.max(0, y));
  const part = await sharp(photo).extract({ left, top, width, height }).toBuffer();
  return sharp(greyCell(w, h)).composite([{ input: part, left: Math.max(0, x), top: Math.max(0, y) }]).png().toBuffer();
}

// match: { tribe, styleDescription, caption, images[], visitorIndex }
// visitor: { buf, box } (the centred cut-out, or the original photo with box = null)
async function composite(match, visitor, viewport) {
  ...
  cells.splice(match.visitorIndex, 0, visitor.buf);
  ...
  const photos = await Promise.all(
    rects.map(async (r, i) => {
      ...
      let input;
      try {
        if (i === match.visitorIndex) input = await visitorTile(visitor, w, h);
        else if (cells[i]) input = await sharp(cells[i]).rotate().resize(w, h, { fit: 'cover', position: 'centre' }).toBuffer();
        else input = await sharp(greyCell(w, h)).png().toBuffer();
      } catch {
